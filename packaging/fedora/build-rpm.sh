...
target="${SECURE_RPM_TARGET:-$root/target/phase69-rpm}"
topdir="$target/rpmbuild"
stage_parent="$target/stage"
stage="$stage_parent/secure-engine-0.1.5"
...
tar --sort=name --mtime='UTC 2026-07-17' --owner=0 --group=0 --numeric-owner -C "$stage_parent" -czf "$topdir/SOURCES/secure-engine-0.1.5.tar.gz" secure-engine-0.1.5
