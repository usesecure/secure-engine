...
The Fedora-first package contains `secure`, `secure-desktop`, desktop launcher metadata, AppStream metadata, a scalable icon, README, and MIT license. It is built entirely below `target/phase4-rpm`; neither build nor verification installs a system package or changes host configuration.
...
```bash
sudo dnf install ./target/phase4-rpm/rpmbuild/RPMS/x86_64/secure-engine-0.1.0-1.fc*.x86_64.rpm
```
