...
- `cargo test --workspace --all-features`: pass; 14 tests cover core inventory, limits, secret/path redaction, mid-scan cancellation, deterministic JSON, precise spans, schema compatibility, shared interfaces, and CLI behavior.
...
- `secure --help`, `secure doctor --format secure-json-v1`, and `secure schema print secure-json-v1`: pass with machine-readable stdout.
- Two scans of `fixtures/integration-project` compare byte-for-byte after removing only `scan.started_at`, `scan.finished_at`, and `scan.duration_ms`.
- The mock Secure Skill consumer validates both the committed v1 fixture and a newly generated report.
- A scan of this workspace validates against the committed schema, exports no absolute workspace path, reports zero findings, and records its Phase 0 limitations.
