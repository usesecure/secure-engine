...
Committed fixtures cover supported, questioned, insufficient, contradicted, malformed, adversarial, prompt-injection, cancellation, timeout, duplicate, redaction, and replay behavior. Verification never performs a live call and uses no real repository data. Phase 6 does not apply fixes, call tools, modify source, add accounts/telemetry/hosted services, add languages/rules/package formats, or claim provider quality.
