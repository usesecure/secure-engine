...
Phases 0–6.9 are complete. Phase 6.9 releases Secure Engine 0.1.5 with generalized remediation derived only from the seven permitted, retired Phase 15 diagnostic artifacts and independent Engine-owned fixtures. It preserves exact source identity and spans through supported aliases, destructuring, helpers, and imports; keeps positional and property-level value connectivity; associates effective guards and sanitizers with the value they protect; and removes overbroad sink-argument propagation. The separately disclosed benchmark adapter defect remains outside Engine. The frozen neutral taxonomy and evidence contract v2 remain read-only. Unaffected historical semantic fingerprints stay stable, affected evidence migrates deterministically, and the parse-cache envelope advances independently. AI validation remains separate and disabled by default. The foundation requirements below remain the historical contract and continue to be enforced by regression tests.
