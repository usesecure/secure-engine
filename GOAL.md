...
Phases 0–6.10 are complete and Phase 6.11 tranche 1 is complete on its development branch. Secure Engine 0.1.6 remains the published release. Phase 6.11 separates bounded local fixed-point convergence from configured interprocedural depth and recognizes fail-closed operation authorization only when it dominates and binds the exact mutation resource. Taxonomy 1.0.0, evidence contract v2, secure-json-v1, SARIF, CLI/desktop parity, and disabled-by-default AI remain unchanged; the private cache is v8 on the development branch. The single known-corpus rescore is retrospective development evidence, not an independent holdout or a release result. Its regressions were preserved and corrected only through independent synthetic fixtures; the final tree was not rescored. No benchmark, ranking, superiority, production-readiness, or complete-coverage claim is supported.
