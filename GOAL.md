...
Phases 0–6.11 are complete and integrated through `44f151308815f3d85a6d7b4f337d46e794fce99a`, which is the exact source base for the Secure Engine 0.1.7 release candidate. Secure Engine 0.1.6 remains the published release. Across three Phase 6.11 tranches the Engine separates bounded local convergence from call depth, binds authorization to the exact mutation resource, resolves final unshadowed dynamic sequence callees, pairs composed filesystem identity with exact confinement, binds constructed redirects to the same URL's fixed origin, and preserves exact outbound property identity. Taxonomy 1.0.0, Evidence Contract v2, secure-json-v1, SARIF, CLI/desktop parity, and disabled-by-default AI remain unchanged; the private development cache is v10. Computed dispatch/properties, reflection, ambiguous imports/calls, and unproven runtime filesystem state remain intentional limits. The known-corpus rescore remains retrospective development evidence and was not repeated; any next evaluation requires a new independent holdout. No benchmark, ranking, superiority, production-readiness, or complete-coverage claim is supported.
