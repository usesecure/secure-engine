...
#[test]
fn cache_v6_ignores_v5_and_reuses_only_current_evidence() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let stale = cache.path().join("secure-parse-cache-v5/legacy/stale.json");
    ...
    assert!(cache.path().join("secure-parse-cache-v6").is_dir());
