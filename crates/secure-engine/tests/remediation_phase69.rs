...
#[test]
fn cache_v10_ignores_older_entries_and_reuses_only_current_evidence()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let stale = cache.path().join("secure-parse-cache-v9/legacy/stale.json");
    ...
    assert!(cache.path().join("secure-parse-cache-v10").is_dir());
